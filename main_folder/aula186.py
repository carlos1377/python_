...
def criar_arquivos(qtd: int, zip_dir: Path):
    for i in range(qtd):
        texto = 'arquivo_%s' % i
        with open(zip_dir / f'{texto}.txt', 'w', encoding='utf8') as file:
            file.write(texto)
...
with ZipFile(CAMINHO_COMPACTADO, 'w') as fzip:
    for root, dirs, files in os.walk(CAMINHO_ZIP_DIR):
        for file in files:
            fzip.write(os.path.join(root, file), file)

# lendo arquivos zip
with ZipFile(CAMINHO_COMPACTADO, 'r') as fzip:
    for file in fzip.namelist():
        print(file)

# desempacotando zip
with ZipFile(CAMINHO_COMPACTADO, 'r') as fzip:
    fzip.extractall(CAMINHO_DESCOMPACTADO)
